...
public interface LotteryRepository extends JpaRepository<Lottery, Long> {
    ...
    @Query("SELECT l FROM Lottery l LEFT JOIN FETCH l.winner WHERE l.ticketsAmountTotal IS NOT NULL AND l.id < ?1 ORDER BY l.id DESC")
    List<Lottery> findFinishedWithWinnersBefore(Long before, Pageable pageable);

    @Query("SELECT l FROM Lottery l LEFT JOIN FETCH l.winner WHERE l.ticketsAmountTotal IS NOT NULL ORDER BY l.id DESC")
