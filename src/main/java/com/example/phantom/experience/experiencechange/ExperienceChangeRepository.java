...
public interface ExperienceChangeRepository extends JpaRepository<ExperienceChange, Long> {
    ...
    @Query("SELECT e FROM ExperienceChange e WHERE e.user.id = ?1 AND e.id < ?2 ORDER BY e.id DESC")
    List<ExperienceChange> findByUserIdBeforePageable(Long userId, Long before, Pageable pageable);
}
