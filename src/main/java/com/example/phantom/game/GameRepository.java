...
public interface GameRepository extends JpaRepository<Game, Long> {
    ...
    @Query("SELECT g FROM Game g JOIN FETCH g.user WHERE g.user.gameHistoryPrivacySetting = ?1 AND g.clientSeed IS NOT NULL ORDER BY g.id DESC")
    List<Game> findHistoryByGameHistoryPrivacySettingWithUsers(PrivacySetting setting, Pageable pageable);

    @Query("SELECT g FROM Game g JOIN FETCH g.user WHERE g.user.gameHistoryPrivacySetting = ?1 AND g.clientSeed IS NOT NULL AND g.id < ?2 ORDER BY g.id DESC")
    List<Game> findHistoryByGameHistoryPrivacySettingWithUsersBefore(PrivacySetting setting, Long before, Pageable pageable);
