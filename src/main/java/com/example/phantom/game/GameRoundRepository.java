...
public interface GameRoundRepository extends JpaRepository<GameRound, Long> {
    ...
    @Query("SELECT r FROM GameRound r WHERE r.user.id = ?1 AND r.gameType = ?2 AND r.clientSeed IS NOT NULL AND r.id < ?3 ORDER BY r.id DESC")
    List<GameRound> findHistoryBefore(Long userId, GameType gameType, Long before, Pageable pageable);

    @Query("SELECT COUNT(r) FROM GameRound r WHERE r.clientSeed IS NOT NULL")
    long countCompleted();

    @Query("SELECT COUNT(r) FROM GameRound r WHERE r.clientSeed IS NOT NULL AND r.timestamp >= ?1")
    long countCompletedSince(Long timestamp);

    @Query("SELECT COUNT(r) FROM GameRound r WHERE r.user.id = ?1 AND r.clientSeed IS NOT NULL")
