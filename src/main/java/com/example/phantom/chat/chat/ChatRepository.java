...
public interface ChatRepository extends JpaRepository<Chat, Long> {
    ...
    @Query("SELECT c.topic.id FROM Chat c WHERE c.topic.id LIKE 'personal-chat-%' AND NOT EXISTS (SELECT 1 FROM TopicMember tm WHERE tm.topic.id = c.topic.id)")
    List<String> findEmptyChatTopicIds(Pageable pageable);

    @Query("SELECT t.id FROM Topic t WHERE t.id LIKE 'personal-chat-%' AND NOT EXISTS (SELECT 1 FROM Chat c WHERE c.topic.id = t.id)")
